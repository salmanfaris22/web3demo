...
class VideoLoaderBlot extends BlockEmbed {
  static create(value: { src: string; id: string }) {
    const node = super.create();
    node.setAttribute("data-loader-id", value.id);
    node.setAttribute("class", "video-loader");

    node.innerHTML = `
      <div style="position: relative; display: inline-block;">
        <video src="${value.src}" controls style="filter: blur(10px); opacity: 0.5; width: 100%; height: auto;"></video>
        <div style="
          position: absolute;
          top: 50%;
          left: 50%;
...
VideoLoaderBlot.blotName = "videoLoader";
VideoLoaderBlot.tagName = "div";
ReactQuill.Quill.register(VideoLoaderBlot);
  ...
  static create(value: { src: string }) {
    ...
    node.innerHTML = `
      <div style="position: relative; display: flex;justify-content:center;">
        <video src="${value.src}" controls style="max-width: 100%; height: auto;"></video>
      </div>
    `;
    return node;
  }
...
icons[
  "video"
] = `<svg class="QuilCustomIcon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M3 3V21L21 12L3 3Z" fill="#A1A19C"/>
</svg>`;
...
export function QuilEditor({
  ...
  const videoHandler = () => {
    ...
    input.onchange = async () => {
      ...
      const insertLoader = (fileUrl: string) => {
        //@ts-ignore
        quillEditor.insertEmbed(range.index, "videoLoader", {
          src: fileUrl,
          id: loaderId,
        });
        //@ts-ignore
      ...
      try {
        const fileUrl = await uploadHandler(file);
        if (fileUrl) {
          replaceLoaderWithVideo(fileUrl, loaderId);
        } else {
          removeLoader(loaderId);
        }
      } catch (error) {
  ...
  const replaceLoaderWithImage = (fileUrl: string, loaderId: string) => {
    const quillEditor = quillRef.current?.getEditor();
    const loaderBlotIndex = findBlotIndexByLoaderId(loaderId);
    if (loaderBlotIndex > -1 && quillEditor) {
      quillEditor.deleteText(loaderBlotIndex, 1, SOURCES.USER);
      quillEditor.insertEmbed(loaderBlotIndex, "image", fileUrl, SOURCES.USER);
      quillEditor.insertText(loaderBlotIndex + 1, "\n", SOURCES.USER);
      quillEditor.setSelection(loaderBlotIndex + 2, 0, SOURCES.USER);
    }
  };

  const replaceLoaderWithVideo = (fileUrl: string, loaderId: string) => {
    const quillEditor = quillRef.current?.getEditor();
    const loaderBlotIndex = findVideoBlotIndexByLoaderId(loaderId);
    if (loaderBlotIndex > -1 && quillEditor) {
      quillEditor.deleteText(loaderBlotIndex, 1, SOURCES.USER);
      // quillEditor.insertEmbed(loaderBlotIndex, "videoTag", fileUrl, SOURCES.USER);
      quillEditor.insertEmbed(loaderBlotIndex, "videoTag", {
        src: fileUrl,
      });
      quillEditor.insertText(loaderBlotIndex + 1, "\n", SOURCES.USER);
      quillEditor.setSelection(loaderBlotIndex + 2, 0, SOURCES.USER);
  ...
  const findLoaderIndex = (loaderId: string): number => {
    ...
    currentContents?.ops?.forEach((op, i) => {
      if (op.insert?.imageLoader?.id === loaderId) {
        console.log(op, loaderId, i);
        loaderIndex = i;
      }
    });

    console.log(currentContents, loaderIndex);

    return loaderIndex;
  };

  const findBlotIndexByLoaderId = (loaderId: string): number => {
    const quillEditor = quillRef.current?.getEditor();
    const delta = quillEditor?.getContents();
    let blotIndex = -1;

    // Traverse through the delta operations
    if (delta) {
      let currentIndex = 0;
      delta.ops?.forEach((op: any) => {
        if (
          op.insert &&
          op.insert.imageLoader &&
          op.insert.imageLoader.id === loaderId
        ) {
          blotIndex = currentIndex;
        }
        // Increase the index by the length of the current op's insert (text or embed)
  ...
  const findVideoBlotIndexByLoaderId = (loaderId: string): number => {
    const quillEditor = quillRef.current?.getEditor();
    const delta = quillEditor?.getContents();
    let blotIndex = -1;
    ...
    if (delta) {
      ...
      delta.ops?.forEach((op: any) => {
        if (
          op.insert &&
          op.insert.videoLoader &&
          op.insert.videoLoader.id === loaderId
        ) {
          blotIndex = currentIndex;
        }
        // Increase the index by the length of the current op's insert (text or embed)
