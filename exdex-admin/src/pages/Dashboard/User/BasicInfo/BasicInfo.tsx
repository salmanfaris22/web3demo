...
const BasicInfo = () => {
  ...
  const uploadFile = async () => {
    try {
      ...
      let url = "background";
      if (bgimage) {
        formData.append("background_photo", bgimage);
      }
      if (profileImage) {
        url = "profile";
      ...
      const response = await profileFileUpload(formData, id, url);
      if (response.status) {
        if (response.data) {
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      setBtnText("Saving...");
      const response = await updateUserInfo(data, id);
      if (response.status) {
        if (response.data) {
          dispatch(hideToastById(10));
          getUserDetailsMethod();
              ...
              <div className={classes.bodyWrap}>
                ...
                <div className={classes.bodyRight}>
                  <div className={classes.formOuter}>
                    ...
                    <div className={classes.formField}>
                      <div className={classes.label}>Describe your Reason</div>
                    ...
                    devices.map((item: any, index: number) => (
                      <div className={classes.deviceItem} key={index}>
                        ...
                        <div className={classes.deviceRight}>
                          <div className={classes.name}>{item.device_type}</div>
                          <div className={classes.des}>{item.device_name}</div>
                          {item.current && (
          ...
          <div className={classes.addressWrap} ref={passwordRef}>
            <OverviewCard>
              <div className={classes.addressInner}>
                ...
                <div className={classes.description}>
                  We recommend that youset a strong password that is at least u
